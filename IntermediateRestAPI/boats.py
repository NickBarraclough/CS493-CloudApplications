    ...
    if request.method == 'POST':
        ...
        if len(content) == 3: # Correct amount of attributes
          ...
          "length": content["length"], "loads": None})
            client.put(new_boat)
            success = {"id": new_boat.key.id, "name": content["name"], "type": content["type"],
          "length": content["length"], "self": (request.url + '/' + str(new_boat.key.id)), "loads": None}
            return (success, 201)
...
def boats_get_loads(boat_id):
    if request.method == 'GET': # Get all of a boat's loads
        boat_key = client.key("boats", int(boat_id))
        boat = client.get(key=boat_key)
        if boat == None: # Boat not found
